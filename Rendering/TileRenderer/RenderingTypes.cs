...
public struct GeoFeature : BaseShape
{
    ...
    public GeoFeature(ReadOnlySpan<Coordinate> c, MapFeatureData feature)
    {
        ...
        var naturalKey = feature.Properties.FirstOrDefault(x => x.Key == EnumMapper.Keys.Natural).Value;
        Type = GeoFeatureType.Unknown;
        if (naturalKey != EnumMapper.Values.NULL)
        {
            if (naturalKey == EnumMapper.Values.Natural_Fell ||
                naturalKey == EnumMapper.Values.Natural_Grassland ||
                naturalKey == EnumMapper.Values.Natural_Heath ||
                naturalKey == EnumMapper.Values.Natural_Moor ||
                naturalKey == EnumMapper.Values.Natural_Scrub ||
                naturalKey == EnumMapper.Values.Natural_Wetland)
            {
                Type = GeoFeatureType.Plain;
            }
            else if (naturalKey == EnumMapper.Values.Natural_Wood ||
                     naturalKey == EnumMapper.Values.Natural_Tree_Row)
            {
                Type = GeoFeatureType.Forest;
            }
            else if (naturalKey == EnumMapper.Values.Natural_Bare_Rock ||
                     naturalKey == EnumMapper.Values.Natural_Rock ||
                     naturalKey == EnumMapper.Values.Natural_Scree)
            {
                Type = GeoFeatureType.Mountains;
            }
            else if (naturalKey == EnumMapper.Values.Natural_Beach ||
                     naturalKey == EnumMapper.Values.Natural_Sand)
            {
                Type = GeoFeatureType.Desert;
            }
            else if (naturalKey == EnumMapper.Values.Natural_Water)
            {
...
public struct PopulatedPlace : BaseShape
{
    ...
    public PopulatedPlace(ReadOnlySpan<Coordinate> c, MapFeatureData feature)
    {
        ...
        for (var i = 0; i < c.Length; i++)
            ScreenCoordinates[i] = new PointF((float)MercatorProjection.lonToX(c[i].Longitude),
                (float)MercatorProjection.latToY(c[i].Latitude));
        var name = feature.Properties.FirstOrDefault(x => x.Key == EnumMapper.Keys.Name).Value;
    ...
    public static bool ShouldBePopulatedPlace(MapFeatureData feature)
    {
        ...
        foreach (var entry in feature.Properties)
            if (entry.Key == EnumMapper.Keys.Place || entry.Key == EnumMapper.Keys.Placement || entry.Key == EnumMapper.Keys.Placement_Forward)
            {
                if (entry.Value == EnumMapper.Values.Place_City || entry.Value == EnumMapper.Values.Place_Town ||
                    entry.Value == EnumMapper.Values.Place_Locality || entry.Value == EnumMapper.Values.Place_Hamlet)
                {
                    return true;
                }
            }
...
public struct Border : BaseShape
{
    ...
    public static bool ShouldBeBorder(MapFeatureData feature)
    {
        ...
        foreach (var entry in feature.Properties)
        {
            if (entry.Key == EnumMapper.Keys.Boundary && entry.Value == EnumMapper.Values.Boundary_Administrative)
            {
                foundBoundary = true;
            }
            if (entry.Key == EnumMapper.Keys.Admin_Level && entry.Value == EnumMapper.Values.Admin_Level_Two)
            {
                foundLevel = true;
            }
