...
public static class Program
{
        ...
        foreach (var (tileId, _) in mapData.Tiles)
        {
            ...
            foreach (var way in mapData.Ways)
            {
                ...
                var featureData = new FeatureData
                {
                    ...
                    PropertyKeys = (totalPropertyCount, new List<EnumMapper.Keys>(way.Tags.Count)),
                    PropertyValues = (totalPropertyCount, new List<EnumMapper.Values>(way.Tags.Count))
                };
                ...
                foreach (var tag in way.Tags)
                {
                    ...
                    featureData.PropertyKeys.keys.Add(EnumMapper.ConvertStringToUniqueKey(tag.Key));
                    featureData.PropertyValues.values.Add(EnumMapper.ConvertStringToUniqueValue(tag.Key, tag.Value, featureData.PropertyKeys.keys[featureData.PropertyKeys.keys.Count - 1]));
                }
                ...
                foreach (var nodeId in way.NodeIds)
                {
                    ...
                    {
                        if (!featureData.PropertyKeys.keys.Contains(EnumMapper.ConvertStringToUniqueKey(key)))
                        {
                            featureData.PropertyKeys.keys.Add(EnumMapper.ConvertStringToUniqueKey(key));
                            featureData.PropertyValues.values.Add(EnumMapper.ConvertStringToUniqueValue(key, value, featureData.PropertyKeys.keys[featureData.PropertyKeys.keys.Count - 1]));
                        }
                    }
            ...
            foreach (var (nodeId, node) in mapData.Nodes.Where(n => !usedNodes.Contains(n.Key)))
            {
                ...
                var featurePropKeys = new List<EnumMapper.Keys>();
                var featurePropValues = new List<EnumMapper.Values>();
                ...
                for (var i = 0; i < node.Tags.Count; ++i)
                {
                    ...
                    featurePropKeys.Add(EnumMapper.ConvertStringToUniqueKey(tag.Key));
                    featurePropValues.Add(EnumMapper.ConvertStringToUniqueValue(tag.Key, tag.Value, featurePropKeys[featurePropKeys.Count - 1]));
                }
    ...
    private struct FeatureData
    {
        ...
        public (int offset, List<EnumMapper.Keys> keys) PropertyKeys { get; init; }
        public (int offset, List<EnumMapper.Values> values) PropertyValues { get; init; }
    }
